...
class Hound_MPPI_HL_Control:
    ...
    def main_loop(self):
        ...
        while not rospy.is_shutdown():
            # TODO: get state information
            if (self.state_init and self.odom_update):
                # ctrl = self.model.inference(self.state) ##
                '''
                Tyler: we might want to seperate update/optimize (found within step) because
                update should ideally run faster than optimize
                
                '''
                # ctrl = step(self, x0, map, use_prev_opt=True) -> torch.Tensor:
